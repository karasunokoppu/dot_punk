...
use crate::{
    GameState,
    core::{
        resource::InWorldTime, setting::game_setting::ONE_MINUTE_SECOND, systems::despawn_screen,
    },
};

pub fn in_world_time_plugin(app: &mut App) {
    app.add_systems(OnEnter(GameState::InGame), (setup_time_view, add_timer))
        .add_systems(
            Update,
            (add_time_for_debug, spend_time).run_if(in_state(GameState::InGame)),
        )
        .add_systems(
            OnExit(GameState::InGame),
            (despawn_screen::<TimeViewMarker>, despawn_timer),
        );
}
...
pub fn setup_time_view(mut commands: Commands, res_time: Res<InWorldTime>) {
    ...
    for (_, mut time_spend_timer) in timer.iter_mut() {
        ...
        if time_spend_timer.timer.just_finished() {
            *res_time = res_time.add_time(0, 1);
            println!(
                "now, it spent one minute!!: {}:{}",
                res_time.hour, res_time.minute
            );
            match time_view_colors.single_mut() {
                Ok((mut time_view_color, _)) => {
                    *time_view_color = BackgroundColor(
                        Color::BLACK.with_alpha(res_time.get_brightness_from_time()),
                    );
                    println!("{}", res_time.get_brightness_from_time());
                }
                Err(e) => {
                    println!("Error {e} is occured!!")
                }
            }
        }
    }
...
pub fn add_timer(mut commands: Commands) {
    commands.spawn(TimeSpendTimer {
        timer: Timer::new(
            Duration::from_secs(ONE_MINUTE_SECOND as u64),
            TimerMode::Repeating,
        ),
    });
}

pub fn despawn_timer(mut comamnds: Commands, timer: Query<(Entity, &TimeSpendTimer)>) {
    ...
    if key_input.just_pressed(KeyCode::ArrowUp) {
        match time_view_colors.single_mut() {
            Ok((mut time_view_color, _)) => {
                ...
                *time_view_color =
                    BackgroundColor(Color::BLACK.with_alpha(res_time.get_brightness_from_time()));
                println!("{}", res_time.get_brightness_from_time());
            }
            Err(e) => {
                println!("Error {e} is occured!!")
