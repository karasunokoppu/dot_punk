...
use crate::in_game::world::{map::{components::{Map, Maps, TeleportNode, WallColliderNode}}, Position, SpriteData};
...
pub fn register_stage001(
    mut r_maps: ResMut<Maps>,
) {
    let map001: Map = Map {
        id: 1,
        ...
        teleport_nodes: vec![
            ...
            TeleportNode {
                id: 1,
                node_position: Position { x: 300.0, y: -200.0 },
                target_map: 0, // テレポート先のマップID
                teleport_position: Position { x: -50.0, y: -100.0 }, // テレポート先の座標
            },
        ],
    };
    // マップデータをロード
